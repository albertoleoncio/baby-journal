...
export function SharedPostCard({ driveId, id, title, date }: { driveId: string; id: string; title: string; date: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/shared/post`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ driveId, folderId: id }) });
        if (!active) return;
        if (res.ok) {
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex items-center gap-3">
            {/* No edit/delete for shared read-only feed */}
          </div>
        </div>
